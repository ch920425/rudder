...
This guide is not a feature checklist. A new organization includes a default agent, so you can use it to run the first useful loop: create the organization, write the request as an issue, assign an owner, run the agent, inspect the evidence, and use Messenger to understand what happened.
...
You can use the default agent in the first organization. Create a new agent later when you need a different responsibility, runtime, or capability boundary.
