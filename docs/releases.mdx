...
- Promoted the Library workspace model for organization files, project resources, agent instructions, and bundled skill references.
- Improved Library file actions, editor tabs, drag behavior, mention tokens, and protected managed entries.
- Tightened Messenger and issue attention behavior, including pagination, self-authored comment filtering, and clearer issue update cards.
- Improved agent run and heartbeat surfaces with better run rows, timestamps, avatars, skill analytics, and timer-admission checks.
- Added richer markdown mention support across comments, chat, Library files, and editor surfaces.
- Polished Desktop and local runtime paths around bundled plugin assets, release staging, and user-facing runtime errors.
