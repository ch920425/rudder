...
组织是 Rudder 的第一层工作空间。人类在组织里定方向、添加 agent、把请求沉淀成 issue，并用评论、运行记录、评审和时间线留下可检查的工作记录。

这篇指南不列功能清单。它带你跑完第一条有用流程：创建组织、添加 agent、把请求写成 issue、分配执行者、运行 agent、检查证据，再用 Messenger 和 Calendar 回看发生了什么。
...
大多数第一个组织只需要一个通用执行 agent 或工程 agent。先别把组织结构设计得太复杂。第一个 agent 只要职责清楚，能接手一条真实 issue。
...
Capabilities 写的是边界，不是简历。它应该告诉 Rudder、其他 agent 和未来 reviewer：这个 agent 适合接什么工作，遇到什么情况要停下来问人。
...
这句话比“擅长工程任务”有用得多，因为它说明了输出方式、验证习惯，以及什么时候需要人介入。
...
Calendar 不只是日程页。在 Rudder 里，它用来查看 agent 运行和人类检查点如何占据时间。
...
最有用的 Calendar 数据来自真实 issue 和 heartbeat runs，不是手动创建的装饰事件。
