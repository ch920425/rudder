...
For Pi + DeepSeek, Pi needs both credentials and a provider/model entry. Native
DeepSeek uses `DEEPSEEK_API_KEY` or `pi /login`. Some Pi model IDs route through
OpenRouter unless the native provider is selected; if Pi reports
`No API key found for openrouter`, either set `OPENROUTER_API_KEY` or add a
native DeepSeek provider/model in `~/.pi/agent/models.json`. Enter the matching
`provider/model` value in Rudder, for example `deepseek/deepseek-chat`, and use
**Test now** as the source of truth.
