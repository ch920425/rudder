...
agent, but it did not address cross-agent overlap. When four or more blocks
overlap in a day column, the week view splits them into tiny horizontal columns
that no longer communicate the time block, title, or owner clearly.
...
- Compact groups that require at least four columns and contain at least four
  underlying events into a `collision_cluster` display item.
...
- Week view does not render four-column overlap groups as unreadable slivers.
