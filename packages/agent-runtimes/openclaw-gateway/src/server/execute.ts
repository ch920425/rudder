...
function buildWakeText(payload: WakePayload, rudderEnv: Record<string, string>): string {
  ...
  const lines = [
    ...
    "4) If RUDDER_WAKE_COMMENT_ID exists, read the issue and comments before acting. A mention requests attention; it does not transfer ownership unless the comment explicitly says so.",
    "5) If issueId exists:",
