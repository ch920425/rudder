...
export const RUDDER_AGENT_OPERATING_CONTRACT = [
  ...
  "- Agents: use `[<agent-name>](agent://<agent-id>)` for reference-only links. Board/operator issue-comment composers use `[<agent-name>](agent://<agent-id>?intent=wake)` for wake-intent mentions; agent-authored comments do not fan out peer wakeups.",
