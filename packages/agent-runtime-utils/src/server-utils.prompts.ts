...
export const RUDDER_AGENT_OPERATING_CONTRACT = [
  ...
  "- Project-linked runs expose `$RUDDER_PROJECT_LIBRARY_ROOT` and `$RUDDER_PROJECT_LIBRARY_PATH` when Rudder knows the current project Library folder.",
  "- Library-backed project resources use `sourceType: \"library\"`; their `locator` is a safe path relative to `$RUDDER_ORG_WORKSPACE_ROOT`, usually under `projects/<project>/` or `docs/`, so read them from `$RUDDER_ORG_WORKSPACE_ROOT/<locator>` when that root is available. Rudder does not use protected system roots such as `agents/`, `artifacts/`, `plans/`, or `skills/` as project Library resources.",
  ...
  "- Durable generated files for project work should be written inside `$RUDDER_PROJECT_LIBRARY_ROOT` when it is available. This includes Markdown, plans, design docs, decision notes, screenshots, images, mockups, reports, CSVs, JSON, HTML, and handoff logs.",
  "- When `$RUDDER_PROJECT_LIBRARY_ROOT` is unavailable, place durable shared files under `$RUDDER_ORG_WORKSPACE_ROOT/projects/<project-or-run>/` when available, preferably through `rudder library file put projects/<project-or-run>/<file> --body-file <path> --json`.",
  ...
  "- Durable shared work output should prefer these managed Library paths instead of ad-hoc folders outside `$RUDDER_ORG_WORKSPACE_ROOT`.",
  "",
