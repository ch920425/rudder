...
- **Worker SDK:** `@rudderhq/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@rudderhq/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@rudderhq/plugin-sdk/testing` — in-memory host harness
...
| Import | Purpose |
|--------|--------|
| `@rudderhq/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@rudderhq/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@rudderhq/plugin-sdk/ui/hooks` | Hooks only |
| `@rudderhq/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@rudderhq/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@rudderhq/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@rudderhq/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@rudderhq/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@rudderhq/plugin-sdk/types` | Worker context and API types (advanced) |
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@rudderhq/plugin-sdk`.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@rudderhq/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@rudderhq/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@rudderhq/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@rudderhq/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@rudderhq/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@rudderhq/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@rudderhq/plugin-sdk/ui`.
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@rudderhq/plugin-sdk/ui";
...
```ts
import { startPluginDevServer } from "@rudderhq/plugin-sdk/dev-server";
